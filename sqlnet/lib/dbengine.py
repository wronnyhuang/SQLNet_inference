...
class DBEngine:
    ...
    def execute(self, table_id, select_index, aggregation_index, conditions, lower=True, table_data=None):
    ...
    # ADDED BY RONNY
    def get_query_raw(self, table_id, select_index, aggregation_index, conditions, table_data, lower=True):
        tid = table_id
        cond_ops = ['=', '>', '<', 'OP']
        agg_ops = ['', 'MAX', 'MIN', 'COUNT', 'SUM', 'AVG']
        schema_re = re.compile(r'\((.+)\)')
        ...
        if where_clause:
            where_str = 'WHERE ' + ' AND '.join(where_clause)
            where_str_raw = 'WHERE ' + ' AND '.join(where_clause_raw)
        # query = 'SELECT {} AS result FROM {} {}'.format(select, table_id, where_str)
        query = 'SELECT {} AS result FROM {} {}'.format(select, table_id, where_str_raw)
        #print query
        return query
