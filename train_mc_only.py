...
if __name__ == '__main__':
    ...
    if not args.rl:
        ...
        # init_acc = epoch_acc(model, BATCH_SIZE, val_sql_data, val_table_data, TRAIN_ENTRY, epoch=None)
        ...
        for i in range(100):
            ...
            print('==> dummy')
            train_acc_qm = epoch_acc(model, BATCH_SIZE, dum_sql_data, dum_table_data, TRAIN_ENTRY, write='dummy', experiment=experiment, epoch=i)
            ...
            if TRAIN_AGG:
                if val_acc[1][0] > best_agg_acc:
                    ...
                    best_agg_idx = i+1
                    torch.save(model.agg_pred.state_dict(), 'saved_model/epoch%d.agg_model%s'%(i+1, args.suffix))
                    torch.save(model.agg_pred.state_dict(), agg_m)
                    if args.train_emb:
                        torch.save(model.agg_embed_layer.state_dict(), 'saved_model/epoch%d.agg_embed%s'%(i+1, args.suffix))
                        torch.save(model.agg_embed_layer.state_dict(), agg_e)
            if TRAIN_SEL:
                if val_acc[1][1] > best_sel_acc:
                    best_sel_acc = val_acc[1][1]
                    best_sel_idx = i+1
                    torch.save(model.sel_pred.state_dict(), 'saved_model/epoch%d.sel_model%s'%(i+1, args.suffix))
                    torch.save(model.sel_pred.state_dict(), sel_m)
                    if args.train_emb:
            ...
            if TRAIN_COND:
                if val_acc[1][2] > best_cond_acc:
                    ...
                    best_cond_idx = i+1
                    torch.save(model.cond_pred.state_dict(), 'saved_model/epoch%d.cond_model%s'%(i+1, args.suffix))
                    torch.save(model.cond_pred.state_dict(), cond_m)
                    if args.train_emb:
                        torch.save(model.cond_embed_layer.state_dict(), 'saved_model/epoch%d.cond_embed%s'%(i+1, args.suffix))
                        torch.save(model.cond_embed_layer.state_dict(), cond_e)
